 ...
 */
function default_1(slice_127, isoJSON) {
    ...
    if (this.formats['127.1'] != undefined) {
        if (this.formats['127.1'].ContentType === 'an') {
            bitmap = tools_1.default.getHex(slice_127.slice(0, 16).toString('ascii'))
                .split('')
                .map(Number);
        ...
        bitmap = tools_1.default.getHex(slice_127.slice(0, 8).toString('hex'))
            .split('')
            .map(Number);
        slice_127 = slice_127.slice(8, slice_127.length);
    }
        ...
        if (bitmap[i] === 1) {
            ...
            if (this_format) {
                if (this_format.LenType === 'fixed') {
                    if (formats_1.default[subField].ContentType === 'b') {
