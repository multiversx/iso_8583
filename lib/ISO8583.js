...
const ISO8583Base_1 = require("./ISO8583Base");
const formats_1 = require("./formats");
const requiredFields_1 = require("./requiredFields");
const requiredEcho_1 = require("./requiredEcho");
const types_1 = require("./types");
const tools_1 = require("./tools");
const take_static_metadata_1 = require("./unpack/take_static_metadata");
const msgTypes_1 = require("./msgTypes");
const H = require("./helpers");
const SpT = require("./specialFields/tools");
const addStaticMetaData_1 = require("./pack/addStaticMetaData");
/**
 ...
 */
class ISO8583 extends ISO8583Base_1.default {
    ...
    static getFieldDescription(fields, customFormats) {
        ...
        if (Array.isArray(fields)) {
            for (const field of fields) {
                const this_format = cFormats[field] || formats_1.default[field];
                if (this_format)
                    descriptions[field] = this_format.Label;
            }
        }
        else {
            const this_format = cFormats[fields] || formats_1.default[fields];
            if (this_format)
                descriptions[fields] = this_format.Label;
        }
        return descriptions;
    }
    ...
    toRetransmit() {
        if (!this.Msg)
            return this.throwMessageUndef();
        const mti = this.Msg['0'];
    ...
    toAdvice() {
        if (!this.Msg)
            return this.throwMessageUndef();
        const mti = tools_1.default.getResType(this.Msg['0']);
        if (!mti)
    ...
    getLenBuffer(len) {
        const buf1 = tools_1.default.getTCPHeaderBuffer(Math.floor(len / 256));
        const buf2 = tools_1.default.getTCPHeaderBuffer(Math.floor(len % 256));
    ...
    getTType() {
        if (!this.Msg)
            return this.throwMessageUndef();
        if (this.Msg['3'])
            return tools_1.default.getTransType(this.Msg['3'].slice(0, 2));
        else
            return tools_1.default.toErrorObject(['transaction type not defined in message']);
    }
    getTransactionType() {
    ...
    getAccType() {
        if (!this.Msg)
            return this.throwMessageUndef();
        if (this.Msg['3'])
            return tools_1.default.getAccType(this.Msg['3'].slice(2, 4));
        else
            return tools_1.default.toErrorObject(['transaction type not defined in message']);
    }
    getAccountTypeFrom() {
    ...
    getAccountTypeTo() {
        if (!this.Msg)
            return this.throwMessageUndef();
        if (this.Msg['3'])
            return tools_1.default.getAccType(this.Msg['3'].slice(4, 6));
        else
            return tools_1.default.toErrorObject(['transaction type not defined in message']);
    }
    getTransStatus() {
        if (!this.Msg)
            return this.throwMessageUndef();
        if (this.Msg['39'])
            return tools_1.default.getTranStatus(this.Msg['39']);
        else
            return tools_1.default.toErrorObject(['transaction status not defined in message']);
    }
    attachTimeStamp() {
        if (!this.Msg)
            return this.throwMessageUndef();
        if (this.Msg['0']) {
            ...
            return tools_1.default.toErrorObject(['mti error']);
    }
    /**
    ...
    validateMessage() {
        ...
        const validateRequiredFields = (0, requiredFields_1.default)(this.Msg, this.requiredFieldsSchema);
                    ...
                    if (this_format) {
                        if (this_format.LenType === 'fixed') {
                            if (this_format.MaxLen === this.Msg[field].length) {
                                ...
                                error = tools_1.default.toInvalidLengthErrorObject(field, this.Msg[field].length);
                            }
                        }
                        else {
                            const thisLen = tools_1.default.getLenType(this_format.LenType);
                            if (!this_format.MaxLen)
                                error = tools_1.default.toErrorObject(['max length not implemented for ', this_format.LenType, field]);
                            if (this.Msg[field] && this.Msg[field].length > this_format.MaxLen) {
                                error = tools_1.default.toInvalidLengthErrorObject(field, this.Msg[field].length);
                            }
                            if (thisLen === 0) {
                                error = tools_1.default.toErrorObject(['field', field, ' has no field implementation']);
                            }
                            else {
                                valid = true;
                            }
                        ...
                        error = tools_1.default.toErrorObject(['field ', field, ' has invalid data']);
                    }
                }
            }
    ...
    validateEcho(iso_send, iso_answer) {
        return (0, requiredEcho_1.default)(this.requiredFieldsSchema, iso_answer, iso_send);
    }
    checkMTI() {
        if (!this.Msg)
            return this.throwMessageUndef();
        if ((0, msgTypes_1.default)(this.Msg['0']))
            return true;
        else
            return false;
    }
    _checkMTI(mti) {
        if ((0, msgTypes_1.default)(mti))
            return true;
        else
            return false;
    }
    ...
    getMti() {
        ...
        if (state) {
            ...
            if (mti === null || mti === undefined) {
                return tools_1.default.toErrorObject(['mti undefined in message']);
            }
            else {
                if (this.checkMTI()) {
                    let _mti;
                    if (!this.Msg['0']) {
                        if (!this.MsgType)
                            return tools_1.default.toErrorObject(['mti undefined on field 0']);
                        else
                            _mti = this.MsgType;
                    }
                    else
                    ...
                    return tools_1.default.toErrorObject(['invalid mti']);
                }
            }
        }
        else {
    ...
    getResMTI() {
        if (this.MsgType) {
            return tools_1.default.getResType(this.MsgType);
        }
    }
    rebuildField(field, bitmapLength) {
        if (!this.Msg)
            return this.throwMessageUndef();
        ...
        if (data && tools_1.default.isXmlEncoded(data)) {
    ...
    // ***tested***
    upackFieldWithBitmap(parentField, bitmaLength) {
        if (!this.Msg)
            return this.throwMessageUndef();
        let dataString = this.Msg[parentField];
        let bitmap_127 = tools_1.default.getHex(dataString.slice(0, bitmaLength)).split('').map(Number);
        ...
        for (let i = 0; i < bitmap_127.length; i++) {
            if (bitmap_127[i] === 1) {
                ...
                let this_format = this.formats[field] || formats_1.default[field];
                if (!this_format)
                    throw tools_1.default.toErrorObject(['field ', field, ' format not implemented']);
                if (this_format.LenType === 'fixed') {
                    this.Msg[field] = dataString.slice(0, this_format.MaxLen);
                    dataString = dataString.slice(this_format.MaxLen, dataString.length);
                }
                else {
                    let thisLen = tools_1.default.getLenType(this_format.LenType);
                    if (!this_format.MaxLen)
    ...
    unpackKeyValueStringField(field) {
        if (!this.Msg)
            return this.throwMessageUndef();
        const dataString = this.Msg[field];
        const data = dataString === null || dataString === void 0 ? void 0 : dataString.split('; ');
        if (data.length < 2) {
            return true;
        }
        // @ts-ignore
        data.reduce((_ignored, s) => {
            const kv = s === null || s === void 0 ? void 0 : s.split('=');
            const k = kv[0];
    ...
    getBmpsBinary() {
        ...
        if (state instanceof Error) {
            ...
            if (!this.Msg['0']) {
                return tools_1.default.toErrorObject('message type error, empty or undefined');
            }
            else {
    ...
    getBitMapHex() {
        ...
        if (state.error) {
            ...
            if (this.bitmaps !== null && (0, msgTypes_1.default)(this.MsgType)) {
                let map = '';
                const maps = [];
                let counter = 0;
                ...
                return tools_1.default.toErrorObject('bitmap error, expecting 128 length unit array');
        }
    }
    getBitMapFields() {
        const bitmap = [];
    ...
    hasSecondaryBitmap(primaryBitmapBuffer, config) {
        ...
        const bitmap = tools_1.default.getHex(binary).split('').map(Number);
        return bitmap[0] === 1;
    }
    /**
    ...
    getIsoJSON(buffer, config) {
        ...
        if (Buffer.isBuffer(buffer)) {
            ...
            buffer = (0, take_static_metadata_1.default)(this, buffer);
            ...
            return tools_1.default.toErrorObject(['expecting buffer but got ', typeof buffer]);
        }
    }
    /**
    ...
    buildBitmapBuffer(bitmap, type) {
        if (type === 'ascii')
            return Buffer.alloc(bitmap.length, bitmap.toUpperCase());
        else
            return Buffer.alloc(bitmap.length / 2, bitmap, 'hex');
    }
    /**
    ...
    getBufferMessage() {
        ...
        const staticMetadataBuf = (0, addStaticMetaData_1.default)(this);
        const _0_127_Buffer = this.assemble0_127_Fields();
        if (_0_127_Buffer instanceof Error) {
            return _0_127_Buffer;
        }
        else {
            const len_0_127_1 = tools_1.default.getTCPHeaderBuffer(Math.floor(_0_127_Buffer.byteLength / 256));
            const len_0_127_2 = tools_1.default.getTCPHeaderBuffer(Math.floor(_0_127_Buffer.byteLength % 256));
    ...
    encode() {
        ...
        if (_0_127_Buffer instanceof Error) {
            ...
            const len_0_127_1 = tools_1.default.getTCPHeaderBuffer(Math.floor(Number(_0_127_Buffer.byteLength) / 256));
            const len_0_127_2 = tools_1.default.getTCPHeaderBuffer(Math.floor(Number(_0_127_Buffer.byteLength) % 256));
            return Buffer.concat([len_0_127_1, len_0_127_2, staticMetadataBuf, _0_127_Buffer]);
            // return Buffer.concat([len_0_127_1, len_0_127_2, _0_127_Buffer]);
        }
    }
    ...
    addField(field, data) {
        if (!this.Msg)
            return tools_1.default.toErrorObject('message undefined');
        const this_format = this.formats[field] || formats_1.default[field];
        if (!this_format)
            return tools_1.default.toErrorObject('field ' + field + ' not implemented');
        const state = (0, types_1.default)(this_format, this.Msg[field].toString(), field);
        if (field === 0 || field === '0') {
            this.Msg['0'] = data;
            this.MsgType = data;
            return true;
